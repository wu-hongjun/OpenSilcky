#!/usr/bin/env bash
#
# build-app.sh — Build OpenSlicky.app bundle and DMG
#
# Usage: bash scripts/build-app.sh <version>
#   e.g. bash scripts/build-app.sh 0.1.0
#
# Expects release binaries at target/release/{slicky,slickyd}.
# Produces  target/release/OpenSlicky.app/  and  OpenSlicky-v<version>-aarch64-apple-darwin.dmg
...
APP="$REPO_ROOT/target/release/OpenSlicky.app"
CONTENTS="$APP/Contents"
MACOS_DIR="$CONTENTS/MacOS"

echo "==> Building OpenSlicky.app (version ${PLIST_VERSION})"
...
# --- Compile SwiftUI launcher (Contents/MacOS/OpenSlicky) ------------------
...
if command -v create-dmg &>/dev/null; then
  TAG="v${PLIST_VERSION}"
  DMG_NAME="OpenSlicky-${TAG}-aarch64-apple-darwin.dmg"
  ...
  echo "==> Building Uninstall OpenSlicky.app..."
  UNINSTALL_APP="$DMG_STAGE/Uninstall OpenSlicky.app"
  ...
  create-dmg \
    --volname "OpenSlicky ${TAG}" \
    --window-size 500 340 \
    --icon-size 80 \
    --app-drop-link 350 120 \
    --icon "OpenSlicky.app" 150 120 \
    --icon "Uninstall OpenSlicky.app" 250 260 \
    --no-internet-enable \
    "$DMG_PATH" \
    "$DMG_STAGE"
